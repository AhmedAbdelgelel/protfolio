...
(async function main() {
  ...
  /* ---------- 6 · regenerate the no-JS fallback in index.html ----------
     the <noscript>-equivalent static block used to be hand-written and
     drifted from cv-data.js (old summary, stale projects). markers make
     index.html the renderer, cv-data.js the single source of truth. */
  const htmlFile = path.join(ROOT, "index.html");
  let html = fs.readFileSync(htmlFile, "utf8");
  const M_START = "<!-- FALLBACK:START (auto-generated from assets/cv/ — do not hand-edit) -->";
  const M_END = "<!-- FALLBACK:END -->";
  const a = html.indexOf(M_START);
  const b = html.indexOf(M_END);
  if (a !== -1 && b !== -1 && b > a) {
    ...
    const fallback =
      M_START + "\n" +
      '    <h1>' + escH(data.name) + '</h1>\n' +
      '    <p class="muted">' + escH(data.role + " · " + data.location) + '</p>\n' +
      '    <p>' + escH(data.summary) + '</p>\n' +
      '\n' +
      '    <h2 class="mono-head">SELECTED PROJECTS</h2>\n' +
      '    <ul>\n' +
      data.projects.map(function (p) {
        return '      <li><strong>' + escH(p.name) + '</strong> — ' +
          escH((p.lines[0] || "").replace(/\.$/, "")) + '.\n' +
          '          <a href="' + escH(p.url) + '" class="mono-link">repo</a></li>';
      }).join("\n") + '\n' +
      '    </ul>\n' +
      '\n' +
      '    <h2 class="mono-head">CONTACT</h2>\n' +
      '    <ul>\n' +
      '      <li>email: <a href="mailto:ahmed4bdelgelel@gmail.com">ahmed4bdelgelel@gmail.com</a></li>\n' +
      '      <li>phone: <a href="tel:+201026657839">+20 102 665 7839</a></li>\n' +
      '      <li>linkedin: <a href="https://www.linkedin.com/in/ahmed-abdelgelel-6aa523283/" target="_blank" rel="noopener">linkedin.com/in/ahmed-abdelgelel-6aa523283</a></li>\n' +
      '      <li>github: <a href="https://github.com/AhmedAbdelgelel" target="_blank" rel="noopener">github.com/AhmedAbdelgelel</a></li>\n' +
      '    </ul>\n' +
      '\n' +
      '    <p class="fallback__note">This page is meant to be a command line — enable JavaScript and type <em>help</em>.</p>\n' +
      '    ' + M_END + '\n' +
      '  ';
    html = html.slice(0, a) + fallback + html.slice(b + M_END.length);
    fs.writeFileSync(htmlFile, html);
    console.log("sync-cv: regenerated no-JS fallback block in index.html");
  }
