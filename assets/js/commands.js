...
(function () {
  ...
  /* cv-data.js (auto-generated by scripts/sync-cv.js from the newest
     assets/cv/*.pdf — rebuilds on every deploy) overrides the fallback:
     drop a new résumé pdf in assets/cv/ and every CV section follows it */
  (function () {
    var d = window.CV_DATA || {};
    ["name", "role", "location", "summary", "experience", "projects", "skills", "education"].forEach(function (k) {
      if (d[k] !== undefined && d[k] !== "") CV[k] = d[k];
    });
    CV.pdf = "assets/cv/" + ((d.meta && d.meta.source) || "AhmedAbdelgelel.pdf");
  })();
  ...
  var commands = {
    ...
    projects: {
      ...
      run: function (ctx) {
        CV.projects.forEach(function (p) {
          ctx.print("<b>" + esc(p.name) + "</b>");
          if (p.url) ctx.print("  <a href=\"" + p.url + "\" target=\"_blank\" rel=\"noopener\" class=\"dim\">" + esc(p.url.replace("https://github.com/", "")) + "</a>");
    ...
    stack: {
      ...
      hash: "stack",
      run: function (ctx) {
        CV.skills.forEach(function (s) { ctx.print(pad(s[0], 11) + esc(s[1])); });
      },
    },
    ...
    experience: {
      ...
      run: function (ctx) {
        CV.experience.forEach(function (job) {
          ctx.job(esc(job.role), esc(job.dates), job.bullets.map(function (b) { return esc(b); }));
        });
      },
    },
  ...
  /* ---------- fun ---------- */
    ...
    cat: {
      ...
      run: function (ctx, rest) {
        ...
        /* cv-backed files — rendered live from the newest pdf in assets/cv/ */
        var section = function (f) {
          var L = [];
          if (f === "skills.txt") {
            CV.skills.forEach(function (s) { L.push(pad(s[0], 11) + s[1]); });
          } else if (f === "education.txt") {
            L.push(CV.education);
          } else if (f === "experience.txt") {
            CV.experience.forEach(function (job) {
              L.push("  " + job.role + " — " + job.dates);
              job.bullets.forEach(function (b) { L.push("    · " + b); });
            });
          } else if (f === "projects.txt") {
            CV.projects.forEach(function (p) {
              L.push("  " + p.name + (p.url ? "  (" + p.url.replace("https://github.com/", "") + ")" : ""));
              p.lines.forEach(function (l) { L.push("    · " + l); });
            });
          } else if (f === "contact.txt") {
            L.push("email " + EMAIL + " · phone " + PHONE + " · " + LINKEDIN + " · " + GITHUB);
          }
          return L;
        };
        if (name && section(name).length) {
          ctx.print(esc(name) + ":");
          section(name).forEach(function (l) { ctx.print('<span class="dim">  ' + esc(l) + "</span>"); });
          return;
        }
        if (name) {
